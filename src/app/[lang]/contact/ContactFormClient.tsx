...
export default function ContactFormClient({
  ...
  function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    const body = [
      ...
      [
        fields.organizationType.label,
        formData["organization-type"],
      ],
      [fields.fleetScale.label, formData["fleet-scale"]],
      [
        fields.primaryInterest.label,
        formData["primary-interest"],
      ],
      [fields.message.label, formData.message],
    ]
      .map(
        ([label, value]) =>
          `${encodeURIComponent(label)}%3A%20${encodeURIComponent(
            value,
          )}`,
      )
      .join("%0A");
    ...
    <div className="bg-white">
      ...
      <Section id="contact-form" className="bg-white">
        <div className="grid gap-10 lg:grid-cols-[0.72fr_1.28fr] lg:gap-20">
          ...
          <form
            className="stagger-item grid min-w-0 gap-5"
